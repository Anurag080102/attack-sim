...
class CryptographicFailuresAttack(BaseOWASPAttack):
    ...
    WEAK_CIPHERS = [
        ...
        "DHE-RSA",
        "DHE-DSS",  # Weak DH params possible
        ...
        "40",
        "56",
        "128",  # Bit strength indicators for weak keys
    ]
    ...
    SENSITIVE_DATA_PATTERNS = [
        ...
        (
            r'user_password["\']?\s*[:=]\s*["\']?[\w@#$%^&*]+',
            "User password in response",
        ),
        ...
        # Encryption keys
        (
            r'encryption[_-]?key["\']?\s*[:=]\s*["\']?[A-Za-z0-9+/=]{32,}',
            "Encryption key",
        ),
        (
    ...
    def _test_ssl_configuration(self, target: str) -> Generator[Finding, None, None]:
        ...
        if supported_deprecated:
            yield Finding(
                ...
                evidence="Supported Deprecated Protocols:\n"
                + "\n".join(
                    [
                        f"  • {proto}: {self.INSECURE_PROTOCOL_NAMES.get(proto, 'Deprecated')}"
