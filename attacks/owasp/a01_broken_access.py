                    ...
                    if response and response.status_code == 200:
                        ...
                        if len(response.text) != baseline_responses[endpoint]["length"]:
                            yield Finding(
                                ...
                                severity=Severity.HIGH,
                                description=f"Endpoint may be vulnerable to Insecure Direct Object Reference. "
                                           f"Different content returned for different IDs without authorization check.",
                                evidence=f"Baseline URL: {baseline_url}, Test URL: {test_url}, "
                                        f"Different response lengths indicate accessible data",
                                remediation="Implement proper authorization checks. Verify the requesting user "
