...
import socket
import math
from typing import Generator, Dict, Any, List, Set, Tuple, Optional
...
class BrokenAccessControlAttack(BaseOWASPAttack):
    ...
    ADMIN_KEYWORDS = {
        ...
        "panel", "console", "config", "configuration", "settings", "setup",
        "system", "internal", "private", "secure", "protected", "restricted",
        "auth", "login", "user", "users", "account", "accounts", "profile",
        "role", "permission", "access", "privilege", "super", "root",
        "backup", "restore", "export", "import", "debug", "test", "dev",
        "staging", "api", "endpoint", "service", "monitor", "log", "logs",
    }

    # Suspicious file extensions for scoring
    SUSPICIOUS_EXTENSIONS = {
        ".bak", ".backup", ".old", ".orig", ".save", ".swp", ".tmp",
        ".config", ".cfg", ".conf", ".ini", ".env", ".sql", ".db",
        ".log", ".logs", ".txt", ".csv", ".xml",
    }
    ...
    PARAM_FUZZ_PATTERNS = [
        ("id[]", "1"), ("id[0]", "1"), ("id[admin]", "true"),
        ("role", "admin"), ("admin", "1"), ("isAdmin", "true"), 
        ("is_admin", "1"), ("type", "admin"), ("user_type", "admin"),
        ("debug", "1"), ("test", "1"), ("dev", "1"), ("admin_mode", "1"),
    ]
    ...
    def _analyze_response_content(self, url: str, response: Any) -> Optional[Finding]:
        ...
        admin_actions = [
            'delete user', 'remove user', 'add user', 'create user',
            'edit permissions', 'modify role', 'grant access',
            'revoke access', 'change password', 'reset password',
    ...
    def _test_http_method_bypass(self, target: str) -> Generator[Finding, None, None]:
        ...
        if vulnerable_methods:
            ...
            method_details = "\n".join([f"- {method}: {len(vulnerable_paths[method])} path(s) ({', '.join(vulnerable_paths[method][:2])}{'...' if len(vulnerable_paths[method]) > 2 else ''})" for method in unique_methods])
    ...
    def _test_forced_browsing(self, target: str) -> Generator[Finding, None, None]:
        ...
        sensitive_files = [
            "/admin.php", "/config.php", "/database.php", "/settings.php",
            "/web.config", "/app.config", "/.env", "/config.json",
        ]
    ...
    def _test_parameter_fuzzing(self, target: str) -> Generator[Finding, None, None]:
        ...
        if vulnerable_params:
            ...
            param_details = "\n".join([f"- {p['parameter']}={p['value']} on {p['endpoint']}" for p in vulnerable_params])
    ...
    def _test_cookie_manipulation(self, target: str) -> Generator[Finding, None, None]:
        ...
        if vulnerable_cookies:
            cookie_details = "\n".join([f"- {list(c['cookies'].keys())[0]}={list(c['cookies'].values())[0]} on {c['path']}" for c in vulnerable_cookies])
    ...
    def run(self, target: str) -> Generator[Finding, None, None]:
        ...
        try:
            ...
            yield Finding(
                ...
                evidence=f"Host is online and accepting connections",
                remediation="N/A - Informational",
                metadata={"hostname": hostname, "port": port},
            )
