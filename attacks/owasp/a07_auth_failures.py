...
class AuthFailuresAttack(BaseOWASPAttack):
        ...
        for weak_pwd in self.WEAK_PASSWORDS[:5]:  # Test first 5
            ...
            if response:
                ...
                if not was_rejected:
                    ...
                    success_patterns = ["success", "created", "registered", "welcome", "verify"]
    ...
    def _test_session_security(self, target: str) -> Generator[Finding, None, None]:
        ...
        if cookies or set_cookie_header:
            ...
            for cookie in cookies:
                ...
                is_session_cookie = any(s in cookie_name_lower for s in session_cookie_names)

                if is_session_cookie or len(cookie_value) > 20:  # Likely a session token
    ...
    def _test_account_lockout(self, target: str, endpoints: Dict) -> Generator[Finding, None, None]:
